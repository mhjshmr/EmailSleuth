    ...
    for part in msg.iter_attachments():
        ...
        if filename:
            ...
            if any(filename.lower().endswith(ext) for ext in risky_types):
                attachment_score += 15  # Adjust weight
...
def generate_report_text(headers, sender_info, sender_findings, auth_findings, urls, url_analysis, image_findings, attachments_list, attachment_findings, threat_score):
...
def main():
    ...
    # Safely add attachment contribution
    threat_score += attachment_score
    if threat_score > 100:
        threat_score = 100
    ...
    urls, url_analysis, image_findings, attachments_list, attachment_findings, threat_score
   )
